...
from pai.common.utils import parse_oss_uri, parse_nas_uri, parse_cpfs_uri, parse_local_file_uri, parse_pai_dataset_uri, \
    parse_odps_uri, parse_bmcpfs_uri
from pai.session import get_default_session, _is_running_in_dlc, \
    _get_current_region_id, _get_dlc_job_id
        ...
        try:
            ...
            if dataset_detail:
                ...
                if dataset_detail.get('Provider') == 'pai':
                    _entity.QualifiedName = f"pai-dataset.{dataset_id}_{dataset_version}".format(
            ...
            logger.warning(f"can not find dataset by dataset_id: {dataset_id}, {str(e)}")
    parsed_result = parse_odps_uri(input_uri)
    if parsed_result:
        project_name, schema, table_name = parsed_result
        _entity = _LineageEntity()
        if schema:
            _entity.QualifiedName = f"maxcompute-table.{project_name}.{schema}.{table_name}".format(
                project_name=project_name, schema=schema, table_name=table_name)
        else:
            _entity.QualifiedName = f"maxcompute-table.{project_name}.{table_name}".format(
...
class Lineage(object):
    ...
    def log_lineage(self, input_entities: List[LineageEntity], output_entities: List[LineageEntity]):
        """
        ...
        if _is_running_in_dlc():
            ...
            if len(_input_entities) == 0 or len(_output_entities) == 0:
                ...
                session.lineage_api.log_lineage(_input_entities, _output_entities, _get_dlc_job_id(),
...
def log_lineage(input_entities: List[LineageEntity], output_entities: List[LineageEntity]):
    """
