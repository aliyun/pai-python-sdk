...
from pai.predictor.service import BuildInProcessor, Service, ServiceConfig
...
class Model(EntityBaseMixin):
    ...
    def get_by_name(cls, name, session: Session = None):
        iter = make_list_resource_iterator(cls.list, name=name, session=session)
...
class ModelVersion(EntityBaseMixin):
    ...
    def _update_inference_spec_by_model_property(self):
        ...
        build_in_processor = BuildInProcessor.get_default_by_model_format(
            self.model_format
        )
