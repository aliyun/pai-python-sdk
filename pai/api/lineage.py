...
class LineageAPI(WorkspaceScopedResourceAPI):
    ...
    def log_lineage(self, inputs: List[_LineageEntity], outputs: List[_LineageEntity], job_id: str, workspace_id: str):
        input_entities = []
        output_entities = []
        for input in inputs:
            input_entities.append(
                LineageEntity(attributes=input.Attributes, entity_type=input.EntityType, name=input.Name,
                              qualified_name=input.QualifiedName))
        for output in outputs:
            output_entities.append(
                LineageEntity(attributes=output.Attributes, entity_type=output.EntityType, name=output.Name,
        ...
        response = self._do_request(
            method_=self._register_lineage, request=request
        )
