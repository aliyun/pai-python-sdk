...
class BuildInProcessor(object):
    ...
    def from_framework_version(
        ...
        if framework_version in versions:
            ...
            logger.warning(
                "Not found the processor for the framework_version({} {}), use the latest processor".format(
...
class ComputeConfig(object):
    ...
    @classmethod
    def default_compute_config(cls):
...
class ServiceConfig(object):
    def __init__(
        ...
        instance_cpu: int = None,
        instance_memory: int = None,
        instance_gpu: int = None,
...
class QueueServiceConfig(object):
    def __init__(self):
        pass
...
class Service(EntityBaseMixin):
    ...
    def scale(self, instance_count=None, compute_target=None, wait=True):
        config = ServiceConfig(
            name=self.name, instance_count=instance_count, compute_config=compute_target
        )
    ...
    def get_predictor(self, serializer=None, network_type="internet"):
        """Get predictor for the service.

        Args:
            network_type: Endpoint network type, could be 'internet' or 'intranet'
            serializer:
    ...
    def get_default_serializer(self):
        ...
        from pai.predictor.serializers import (
            BaseSerializer,
            JsonSerializer,
            TorchSerializer,
        )
    ...
    def build_tensorflow_serializer(self):
        ...
        from pai.predictor.serializers import TensorFlowSerializer
...
class ServiceGroup(EntityBaseMixin):
    pass
