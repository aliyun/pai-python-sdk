...
from pai.predictor.service import ComputeConfig, Service, ServiceStatus
...
class TestEasServiceAPI(BaseIntegTestCase):
    ...
    def test_blue_green_deploy(self):
        ...
        # Blue service predictor is the entry of the blue-green service group,
        # and it should be alive after one of backend service instance is deleted.
        self.assert_endpoint_valid(blue_service_predictor)
    ...
    #     for s in Service.list(page_size=50):
    #         if (
    #             s.status == ServiceStatus.Running or s.status == ServiceStatus.Failed
    #         ) and (s.name.startswith("sdktest") or s.name.startswith("example_")):
    #             print(s.name)
    #             s.delete()
