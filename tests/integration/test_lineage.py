...
from pai.tracking import log_lineage, LineageEntity
...
class TestLineage(BaseIntegTestCase):
    ...
    def test_run_in_non_dlc_env(self):
        with self.assertLogs(logger=get_logger('pai.tracking.lineage'), level=logging.WARNING) as captured:
            log_lineage(input_entities=[
                LineageEntity(uri='pai://datasets/d-f0mniq7j4cgk2x2rrn/v1', resource_type='dataset',
                              resource_use='train'),
            ], output_entities=[
                LineageEntity(uri='file:///mnt/model/', resource_type='model', resource_use='extension')])
            self.assertIn("log_lineage is not supported in non-DLC environment.", captured.output[0])
    ...
    def test_log_lineage_with_no_var_datasources_file_in_dlc(self):
        with self.assertLogs(logger=get_logger('pai.tracking.lineage'), level=logging.WARNING) as captured:
            log_lineage(
                input_entities=[
                    LineageEntity(uri='file:///mnt/models/model.pth', resource_type='model', resource_use='extension')],
                output_entities=[LineageEntity(uri='oss://test-bucket.oss-cn-shanghai.aliyuncs.com/model/model.pth',
                                               resource_type='model', resource_use='extension')])
            self.assertIn(
    ...
    def test_valid_log_lineage_in_dlc(self):
        with self.assertLogs(logger=get_logger('pai.tracking.lineage'), level=logging.DEBUG) as captured:
            log_lineage(input_entities=[
                LineageEntity(uri='oss://test-bucket.oss-cn-shanghai.aliyuncs.com/models/ALBERTv2-Chinese-NewsBase.pth',
                              resource_type='model',
                              resource_use='base'),
                LineageEntity(uri='nas://fsId-mountTarget.cn-hangzhou.nas.aliyuncs.com/nas/mountTarget/',
                              resource_type='dataset',
                              resource_use='train'),
                LineageEntity(uri='cpfs://cpfs-0077f18ed141a84e.cn-hangzhou/ptc-00f31da01c2a9c12/exp-005607872325f692/',
                              resource_type='dataset',
                              resource_use='train'),
                LineageEntity(uri='bmcpfs://cpfs-291070fd9529c747-000001.cn-wulanchabu.cpfs.aliyuncs.com/',
                              resource_type='dataset',
                              resource_use='train'),
                LineageEntity(uri='pai://datasets/d-jipftzxinc7nm1z0uh/v1', resource_type='dataset',
                              resource_use='train'),
                LineageEntity(uri='odps://project_mc/tables/flow_model_label_table_v1', resource_type='dataset',
                              resource_use='test')
            ], output_entities=[
                LineageEntity(uri='oss://hangzhoutest01.oss-cn-hangzhou-internal.aliyuncs.com/models/model.pth',
                              resource_type='model', resource_use='extension')])
            self.maxDiff = None
            self.assertEquals(
    ...
    def test_log_lineage_with_invalid_format_input_entities_in_dlc(self):
        with self.assertLogs(logger=get_logger('pai.tracking.lineage'), level=logging.WARNING) as captured:
            log_lineage(input_entities=[
                LineageEntity(uri='oss://test-bucket/models/ALBERTv2-Chinese-NewsBase.pth', resource_type='model',
                              resource_use='base'),
                LineageEntity(uri='pai://datasets/', resource_type='dataset', resource_use='train'),
                LineageEntity(uri='odps://project_mc/flow_model_label_table_v1', resource_type='dataset',
                              resource_use='test')
            ], output_entities=[
                LineageEntity(uri='oss://test-bucket.oss-cn-shanghai.aliyuncs.com/model/model.pth',
                              resource_type='model', resource_use='extension')])
            self.assertIn("input_entities or output_entities is empty, ignore.", captured.output[0])
    ...
    def test_log_lineage_with_invalid_format_output_entities_in_dlc(self):
        with self.assertLogs(logger=get_logger('pai.tracking.lineage'), level=logging.WARNING) as captured:
            log_lineage(input_entities=[
                ...
                LineageEntity(uri='pai://datasets/', resource_type='dataset', resource_use='val'),
                LineageEntity(uri='odps://project_mc/flow_model_label_table_v1', resource_type='dataset',
                              resource_use='test')
            ])
            self.assertIn("input_entities or output_entities is empty, ignore.", captured.output[0])
    ...
    def test_log_lineage_with_empty_input_output_entities_in_dlc(self):
        with self.assertLogs(logger=get_logger('pai.tracking.lineage'), level=logging.WARNING) as captured:
            log_lineage([], [])
            self.assertIn("input_entities or output_entities is empty, ignore.", captured.output[0])
            log_lineage(input_entities=[LineageEntity(uri='')], output_entities=[LineageEntity(uri='')])
            self.assertIn("input_entities or output_entities is empty, ignore.", captured.output[1])
